...
enum PCD_Command {
  ...
  PCD_NoCmdChange = 0x07,  // no command change, can be used to modify the CommandReg register bits
                           // without affecting the command, for example, the PowerDown bit
  PCD_Receive = 0x08,      // activates the receiver circuits
  PCD_Transceive = 0x0C,   // transmits data from FIFO buffer to antenna and automatically activates
...
enum PICC_Command {
  // The commands used by the PCD to manage communication with several PICCs (ISO 14443-3, Type A,
  // section 6.4)
  PICC_CMD_REQA = 0x26,  // REQuest command, Type A. Invites PICCs in state IDLE to go to READY and
                         // prepare for anticollision or selection. 7 bit frame.
  PICC_CMD_WUPA = 0x52,  // Wake-UP command, Type A. Invites PICCs in state IDLE and HALT to go to
  ...
  // The commands used for MIFARE Classic (from http://www.mouser.com/ds/2/302/MF1S503x-89574.pdf,
  // Section 9)
  ...
  PICC_CMD_MF_READ = 0x30,   // Reads one 16 byte block from the authenticated sector of the PICC.
                             // Also used for MIFARE Ultralight.
  PICC_CMD_MF_WRITE = 0xA0,  // Writes one 16 byte block to the authenticated sector of the PICC.
                             // Called "COMPATIBILITY WRITE" for MIFARE Ultralight.
  PICC_CMD_MF_DECREMENT = 0xC0,  // Decrements the contents of a block and stores the result in the
                                 // internal data register.
  PICC_CMD_MF_INCREMENT = 0xC1,  // Increments the contents of a block and stores the result in the
                                 // internal data register.
...
// Return codes from the functions in this class. Remember to update GetStatusCodeName() if you add
// more. last value set to 0xff, then compiler uses less ram, it seems some optimisations are
// triggered
typedef enum {
  STATUS_OK,                 // Success
...
StatusCode PICC_Select(spi_device_handle_t spi, mfrc522_uid* uid, uint8_t validBits);
