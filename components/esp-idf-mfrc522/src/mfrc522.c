...
StatusCode PCD_CommunicateWithPICC(
  ...
  if (backData && backLen) {
    ...
    *backLen = n;  // Number of uint8_ts returned
    ...
    _validBits =
        PCD_ReadRegister(mfrc, ControlReg) & 0x07;  // RxLastBits[2:0] indicates the number of valid
                                                    // bits in the last received uint8_t. If this
                                                    // value is 000b, the whole uint8_t is valid.
    if (validBits) {
  ...
  while (!uidComplete) {
    ...
    switch (cascadeLevel) {
      case 1:
        ...
        uidIndex = 0;
        useCascadeTag =
            validBits && uid->size > 4;  // When we know that the UID has more than 4 uint8_ts
        break;
      ...
      case 2:
        ...
        uidIndex = 3;
        useCascadeTag =
            validBits && uid->size > 7;  // When we know that the UID has more than 7 uint8_ts
        break;
    ...
    while (!selectDone) {
      ...
      result = PCD_TransceiveData(
          mfrc, buffer, bufferUsed, responseBuffer, &responseLength, &txLastBits, rxAlign, false
      );
      if (result == STATUS_COLLISION) {  // More than one PICC in the field
                                         // => collision.
        uint8_t valueOfCollReg =
            PCD_ReadRegister(mfrc, CollReg);  // CollReg[7..0] bits are: ValuesAfterColl
...
StatusCode PCD_Authenticate(
  ...
  return PCD_CommunicateWithPICC(
      mfrc, PCD_MFAuthent, waitIRq, &sendData[0], sizeof(sendData), NULL, 0, 0, 0, false
  );
...
)  // Authenticate with 32bit password
{
  ...
  result = PCD_CommunicateWithPICC(
      mfrc, PCD_Transceive, waitIRq, cmdBuffer, 7, cmdBuffer, &rxlength, &validBits, 0, false
  );
...
bool MIFARE_OpenUidBackdoor(MFRC522Ptr_t mfrc, bool logErrors) {
  ...
  StatusCode status = PCD_TransceiveData(
      mfrc, &cmd, (uint8_t)1, response, &received, &validBits, (uint8_t)0, false
  );  // 40
  if (status != STATUS_OK) {
    if (logErrors) {
      printf(
  ...
  if (status != STATUS_OK) {
    if (status == STATUS_TIMEOUT) {
      ...
      if (!PICC_IsNewCardPresent(mfrc) || !PICC_ReadCardSerial(mfrc)) {
        printf(
            "No card was previously selected, and none are "
      ...
      if (status != STATUS_OK) {
        ...
        if (logErrors) {
          printf(
              "Failed to authenticate to card for reading, "
              "could not set UID: \r\n"
          );
          printf(GetStatusCodeName(status));
        }
...
bool MIFARE_UnbrickUidSector(MFRC522Ptr_t mfrc, bool logErrors) {
  ...
  uint8_t block0_buffer[] = {
      0x01, 0x02, 0x03, 0x04, 0x04, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00
  };
