...
| 3 | P1, next | [04: Counter reads and retries](04-counter-read-and-retry-errors.md) | Issue 03 removes false-success/zero driver reads, but callers still ignore failures and can request duplicate increments from stale counters. Once hardware exceeds the saved target, further re-presentations cannot complete it. |
...
For firmware without the fixes, the first suspects remain 01, 03, and 04. After validating and deploying 01 and 03, prioritize 04 for immediate re-presentation failures, then establish whether another operation changed the card as in 05. Simple interruption alone is not sufficient to explain persistent failure under otherwise healthy recovery conditions.
