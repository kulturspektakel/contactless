...
| 4 | P1, next | [05: Stale transaction retries](05-stale-transaction-retries.md) | Issue 04 rejects unexplained counters before mutation. Still compare fresh payload contents with the pending transaction: an unrelated operation can leave the same counter with different monetary values. |
...
For firmware without the fixes, the first suspects remain 01, 03, and 04. After validating and deploying these fixes, prioritize issue 05's remaining payload checks and issue 07's sleep protection. Simple interruption alone is not sufficient to explain persistent failure under otherwise healthy recovery conditions.
